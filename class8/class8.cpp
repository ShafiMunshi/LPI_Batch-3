...
int main()
{
    ...
    int i = 1;
    for (; i <= 10; i++)
    { // here semiclon should be given else our program will get an error
        cout << i << endl;
    }
    ...
    {
        if (i == 7 or i == 9)
        { // for this statement our program will ignore 7 and 9;
            continue;
        }
        cout << i << endl;
    }
    ...
    for (int i = 2; i < n; i++)
    { // this loop will continue from 2 to n-1;
        if (n % i == 0)
        {                  // this if will check our n is divible by i or not
    ...
    if (check == true)
    { // this if will check our check is still storing treu?
